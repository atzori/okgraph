"""
The 'sliding_windows' module contains the utilities used to inspect the corpus
and search for semantic similarities between words.
"""
import itertools
import math
import numpy as np
...
class SlidingWindows:
    ...
    def __init__(self,
        ...
        :param windows_size: size of the windows (in 'words') that can contain the target word/words
        ...
        if len(target_words) > windows_size:
            raise ValueError(f"target_words contains {len(target_words)} words that cannot fit"
                             f" in windows of the specified size ({windows_size})")
        if not isinstance(windows_size, int):
            raise TypeError(f"windows_size must be an int")

        self.target_words = list(target_words)
        self.windows_size = windows_size
        logger.info(f"{self.target_words}: Start windowing of target words")

        # Get the corpus data
        self.corpus_index_path = corpus_index_path
        logger.info(f"{self.target_words}: Loading corpus dictionary")
        self.corpus_dict = dict(np.load(corpus_dictionary_path, allow_pickle=True).item())

        # Processing the corpus data
        logger.info(f"{self.target_words}: Processing corpus data")
        logger.debug(f"{self.target_words}: Total unique corpus words: {len(self.corpus_dict)}")
        self.corpus_total_occurrences = self.__total_occurrences(self.corpus_dict)
        logger.debug(f"{self.target_words}: Total corpus occurrences: {self.corpus_total_occurrences}")
        logger.debug(f"{self.target_words}: Building corpus inverse frequency dictionary")
        self.corpus_inverse_frequency_dict = self.__inverse_frequency_dictionary(
            self.corpus_dict, self.corpus_total_occurrences)

        # Create the windows
        logger.info(f"{self.target_words}: Creating windows")
        (self.windows_list, self.windows_dict) = self.__create_windows()

        # Processing windows data
        logger.info(f"{self.target_words}: Processing windows data")
        logger.debug(f"{self.target_words}: Number of windows: {len(self.windows_list)}")
        if len(self.windows_list) > 0:
            logger.debug(f"{self.target_words}: Removing target words from windows dictionary")
            for word in self.target_words:
                self.windows_dict.pop(word)
            logger.debug(f"{self.target_words}: Total unique windows words: {len(self.windows_dict)}")

            self.windows_total_occurrences = self.__total_occurrences(self.windows_dict)
            logger.debug(f"{self.target_words}: Total windows occurrences: {self.windows_total_occurrences}")

            logger.debug(f"{self.target_words}: Building windows occurrence dictionary")
            self.windows_occurrence_dict = self.__windows_occurrence_dictionary(self.windows_dict, self.windows_list)

            logger.debug(f"{self.target_words}: Building windows frequency dictionary")
            self.windows_frequency_dict = self.__frequency_dictionary(self.windows_dict, self.windows_total_occurrences)

            logger.debug(f"{self.target_words}: Building windows noise dictionary")
            self.noise_dict = self.__noise_dictionary(self.windows_dict, self.windows_total_occurrences,
                                                      self.corpus_dict, self.corpus_total_occurrences)

            logger.debug(f"{self.target_words}: Building windows TF-IDF dictionary")
            ...
            logger.debug(f"{self.target_words}: Cleaning windows TF-IDF dictionary")
            ...
            logger.debug(f"{self.target_words}: Sorting cleaned TF-IDF dictionary to obtain labels")
    ...
    def __noise_dictionary(windows_dictionary: {str: int}, windows_total_occurrences: {str: int},
        ...
        for word in windows_dictionary_keys:
            ...
            if word not in corpus_dictionary:
                ...
                noise = np.log10(1+corpus_frequency / window_frequency)
        ...
        for word in windows_frequency_dictionary_keys:
            ...
            if word not in windows_frequency_dictionary or \
               ...
               word not in corpus_inverse_frequency_dictionary:
                ...
                idf = np.log10(corpus_inverse_frequency_dictionary.get(word))
    ...
    def __clean_results(target_words: [str], windows_dictionary: {str: int}, tf_idf_dictionary: {str: float},
        ...
        count_removed_words = 0
        logger.debug(f"{target_words}: Removing words shorter than 3 characters")
        clean_dictionary_keys = list(cleaned_dictionary.keys())
        for key in clean_dictionary_keys:
            if len(key) < 3:
                count_removed_words += 1
                logger.debug(f"{target_words}: Removing '{key}': len={len(key)} < 3")
                cleaned_dictionary.pop(key)
        logger.debug(f"{target_words}: Removed {count_removed_words} words")
        ...
        logger.debug(f"{target_words}: Final cleaned dictionary keys: {cleaned_dictionary}")
