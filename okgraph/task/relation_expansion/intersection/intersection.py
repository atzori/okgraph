...
def task(seed: [(str, str)], k: int, options: dict):
    ...
    logger.debug(f"Importing relation labeling algorithm {relation_labeling_algo}")
    ...
    logger.debug(f"Importing set expansion algorithm {set_expansion_algo}")
    ...
    logger.debug(f"Expansion of first elements list: {first_elements_expansion}")
    logger.debug(f"Expansion of second elements list: {second_elements_expansion}")
    ...
    logger.debug(f"Possible new pairs: {new_pairs}")
    ...
    for pair in new_pairs:
        ...
        logger.debug(f"Pair {pair} labels: {pair_labels}")
        if set(seed_labels) & set(pair_labels) == set(seed_labels):
            logger.debug(f"Valid pair {pair}: seed labels {seed_labels} are contained in {pair_labels}")
            pairs_expansion.append(pair)
        else:
            logger.debug(f"Invalid pair {pair}: seed labels {seed_labels} are not contained in {pair_labels}")

    pairs_expansion = pairs_expansion[:k]
    logger.debug(f"Expansion is {pairs_expansion}")
