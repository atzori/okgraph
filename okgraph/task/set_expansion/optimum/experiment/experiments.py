...
def is_similar(okg: okgraph.OKgraph, doesnt_match_word: list, words: list):
    ...
    simil = []
    for w in words:
        simil.append(okg.embeddings.similarity(doesnt_match_word, w))
...
def remove_words_doesnt_match(okg: okgraph.OKgraph, words: list, initial_guesses: list, max: int = 1, verbose=False):
    ...
    while to_remove and tmp_max > 0:
        tmp_max -= 1
        doesnt_match_word = okg.embeddings.doesnt_match(w2)
...
def run_experiments(models: list,
    ...
    for embeddings_magnitude_model in models:
        ...
        for ground_truth_name in ground_truths:
            ...
            ground_truth_without_not_exists = [e for e in ground_truth if e in okg.embeddings]
            ground_truth_missing_on_we = [e for e in ground_truth if e not in okg.embeddings]
            ...
            for one_model_name in ["GoogleNews-vectors-negative300", "glove.6B.300d", "glove.840B.300d",
                                   "wiki-news-300d-1M"]:
                enable_most_similar_approx = enable_most_similar_approx or (
                            one_model_name in embeddings_magnitude_model)
            ...
            for initial_guesses in initial_guesses_list:
                ...
                if len(initial_guesses) <= 0:
                    print(
                        f'ERROR: Cannot calculate with an initial guesses length equal to zero [{embeddings_magnitude_model}] [{ground_truth_name}].')
                ...
                for optim_algo in optim_algos:
                    ...
                    for objective_metric in objective_metrics:
                        ...
                        globals()['thread_list'].append(
                            threading.Thread(name="T" + str(n), target=get_optimum, args=(args,)))
...
def experiment_t1(args_dict):
    ...
    while another_loop_cmon:
        ...
        print(
            f'\r Threads started [{thread_list_index}] {tmp_chr} {thread_list_index}/{TOTAL_THREADS_TO_RUN} threads ({len(l)} running)',
            end='')
