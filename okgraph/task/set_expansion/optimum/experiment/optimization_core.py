...
def get_similar_and_results(okg: okgraph.OKgraph,
    ...
    if enable_most_similar_approx is True:
        the_most_similar = okg.embeddings.most_similar_approx(vector, topn=topn)
    else:
        the_most_similar = okg.embeddings.most_similar(vector, topn=topn)
...
def create_objective(okg, ground_truth_set, ground_truth_set_vectors, dataset_info=dict(),
    ...
    def objective(x: [np.float64]):  # -> numpy.float64
        ...
        if verbose:
            print(
                f'\r\t\t\t\t\tit: [{iterations}] - {objective_metric} {max_avgp} / actual[{abs(avgp)}] \t ({len(the_most_similar_words)}):{the_most_similar_words[:3]}...',
                end='')
        return 1 - abs(avgp)
...
def get_optimum(args_dict):
    ...
    if verbose == True:
        ...
        print(
            f'\t### Starting #{num_started_exp} optimization enable_most_similar_approx:{enable_most_similar_approx} of ({len(ground_truth)}) : {ground_truth[:3]}...'
    ...
    if len(initial_guesses) <= 0:
        print(
            f'ERROR: Cannot calculate with an initial guesses length equal to zero [{objective_metric}] [{we_model}] [{ground_truth_name}] [{optim_algo}].')
        return
    ...
    if optim_algo is None:
        ...
        solution = so.minimize(objective, x0, method='Newton-CG', jac=rosen_der, hess=rosen_hess,
