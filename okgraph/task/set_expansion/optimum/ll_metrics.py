...
def normalized_discounted_cumulative_gain(serp):
    if float(ideal_discounted_cumulative_gain(serp)) == 0:
        return 0
...
def calculate_y_scores(length):
    out_list = [x/length for x in range(length+1)][1:]
    out_list.reverse()
    return out_list


def get_print_and_save_calculus(filename, dataset_info, results, save_key="", verbose=False):
    ...
    with open(filename, mode='a') as my_csv_data:
        ...
        dataset_info['INFO'] = save_key
        out_calc = calculate(dataset_info, results, verbose)
        titles = []
        results = []
        title_before = ''
...
def calculate(dataset_info: dict = {}, results: [bool] = [], verbose: bool = False):
    ...
    out_calc['PRECISION'] = precision(results)
    out_calc['RECALL'] = recall(results)
    out_calc['Interp_Prec'] = interpolated_precision(results, out_calc['PRECISION'])
    out_calc['AP'] = avg_precision(results, out_calc['PRECISION'])
    ...
    for actual_k in [5, 10, 20, 50, 100]:
        ...
        out_calc[f'Pa{actual_k}'] = precision_at_k(actual_k, out_calc['PRECISION'])
    out_calc['CG'] = cumulative_gain(results)
    out_calc['DCG'] = discounted_cumulative_gain(results)
    out_calc['IDCG'] = ideal_discounted_cumulative_gain(results)
    out_calc['NDCG'] = normalized_discounted_cumulative_gain(results)

    sklearn_score_averages_keys = []
    if dataset_info['sklearn_metric_ap_score_enabled'] is True:
        y_trues = np.array(results)
        y_scores = np.array(calculate_y_scores(len(y_trues)))
