from okgraph.task.set_expansion.optimum.ll_metrics import *
import scipy.optimize as so


def get_similar_and_save_results(okg, filename, vector, topn, save_key, ground_truth, dataset_info, enable_most_similar_approx=False):
    ...
    return get_print_and_save_calculus(filename, dataset_info, p_list, save_key=save_key)
...
def create_objective(okg, ground_truth_set, ground_truth_set_vectors, dataset_info=dict(),
    ...
    def objective(x: [np.float64]): #-> numpy.float64
        ...
        print(f'...', end='')
        avgp = calculate(dataset_info, true_false_list, verbose=False)[objective_metric]
...
def get_optimum(okg, dataset_info: dict, choose_x0_closure: callable, filename: str):
